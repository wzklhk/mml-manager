...
前端没有变化且已有 `backend/src/app/static` 构建产物时，可使用 `.\scripts\run.ps1 start` 或 `./scripts/run.sh start` 跳过打包；日常开发使用 `.\scripts\run.ps1 dev` 或 `./scripts/run.sh dev`，由 Vite 提供热更新。
