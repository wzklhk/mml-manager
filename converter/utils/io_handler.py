...
def read_mml_file(file_path: str, encoding: str = 'utf-8') -> MmlDataSet:
    """
    读取MML文件并解析为数据集。
...
def write_mml_file(file_path: str, dataset: MmlDataSet,
                   source_desc: str = '',
                   cmd_type: str = 'SET',
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        ...
        for table_name in tables:
            ...
            for config in group.configs:
                line = format_mml_command(config.cmd_type, config.table, config.values)
                f.write(line + '\n')
            f.write('\n')
...
def print_statistics(dataset: MmlDataSet, title: str = "解析完成"):
    """
    打印转换统计信息。
...
def ensure_output_path(input_path: str, output_arg: Optional[str],
                       default_ext: str = '.xlsx') -> str:
    """
    确定输出文件路径。
