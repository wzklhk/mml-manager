...
def read_csv(file_path: str, table_name: Optional[str] = None, pick_fields: Optional[List[str]] = None) -> MmlDataSet:
    """
    从CSV文件读取数据为MML数据集。
    ...
    if table_name is None:
        table_name = re.sub(r"[^a-zA-Z0-9_]", "_", os.path.splitext(os.path.basename(file_path))[0])
...
def read_csv_batch(directory: str, output_dir: str, cmd_type: str = "SET") -> MmlDataSet:
    """
    批量读取目录下所有CSV文件。
    ...
    for i, csv_file in enumerate(csv_files, 1):
        ...
        from .io_handler import write_mml_file

        write_mml_file(output_path, ds, source_desc=f"csv2mml.py 从 {csv_file}", cmd_type=cmd_type)
        print()
