...
<template>
  <aside class="module-sidebar" :class="{ collapsed }">
    <button class="sidebar-toggle" :aria-expanded="!collapsed" :aria-label="t('workspace.toggleMenu')" @click="collapsed = !collapsed">
    ...
    <template v-if="!collapsed">
      ...
      <nav :aria-label="t('workspace.functions')">
        <template v-for="item in module.menu || []" :key="item.id">
          ...
          <button v-else class="module-menu-item" disabled>{{ t(item.title) }}<small>{{ t('workspace.soon') }}</small></button>
        </template>
      </nav>
